...
export const CHART_TYPE_PROMPTS = {
  ...
  1. Shows clear trend over time
  2. Highlights key inflection points
  3. Includes proper date/time formating on the x-axis
  ...
  "bar": `
  Analyze this categorical data and create a group column chart that:
  1. Shows vertical comparisons between categories
  2. Display clear value relationships
...
export const SYSTEM_PROMPT = `
  You are a data visualization expert. Your role is to analyze data and create clear, meaningful visualizations using generate_graph_data tool:
  ...
  For Time-series (line/bar):
  {
    chart: {
      ...
      yAxis: "Age",
      data: [
        {[xAxis]: "Arsenal F.C.", [yAxis]: 80},
        {[yAxis]: "Chelsea", [yAxis]: 80},
      ]
    }
  }

  For Distribution (pie):
  {
  ...
  - Structure data exactly as needed for the choosen chart type
  - Use the specific visualization for the data
...
export const sqlPromptMessage = (schema: string, userPrompt: string): string => {
  const content = `
      You are a SQL expert. Base on this database schema:
      ${schema}
